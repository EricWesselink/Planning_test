...
<div class="days" style="min-height: {{ $height }}px">
    <div class="day-grid" aria-hidden="true">
        @foreach ($days as $day)
            <i @class(['is-week' => $day->isMonday()])></i>
        @endforeach
    </div>
    <div class="bars" style="min-height: {{ $height }}px">
        ...
        @foreach ($personBars as $index => $personBar)
            <div class="bar{{ ! empty($personBar['has_budget_overrun']) ? ' is-over' : '' }}"
                 style="background: {{ isset($printShade) ? $printShade($personBar) : $personBar['color'] }}; top: {{ 4 + ($index * 20) }}px; width: calc(({{ $personBar['bar']['span'] }} - {{ $personBar['bar']['start_offset'] ?? 0 }} - (1 - {{ $personBar['bar']['end_offset'] ?? 1 }})) * 100% / {{ $dayCount }} - 2px); left: calc(({{ $personBar['bar']['start'] }} + {{ $personBar['bar']['start_offset'] ?? 0 }}) * 100% / {{ $dayCount }} + 1px);">
