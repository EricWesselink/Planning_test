        ...
        <table class="w-full text-sm">
            ...
            @forelse ($calculations as $calculation)
                <tr class="border-t border-nicon-line">
                    <td class="px-3 py-2">
                        <a class="font-medium text-nicon-orange-dark" href="{{ route('calculations.board', $calculation) }}">{{ $calculation->name }}</a>
                        <div class="text-xs text-nicon-muted">{{ $calculation->drawings_count }} tekening(en) · {{ $calculation->lines_count }} regels</div>
                    </td>
                    ...
                    <td class="px-3 py-2">{{ $calculation->status->label() }}</td>
                    <td class="px-3 py-2 text-right">
                        <a href="{{ route('calculations.board', $calculation) }}" class="text-sm text-nicon-orange-dark">Openen</a>
